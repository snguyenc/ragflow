...
FACTORY = {
    ...
    ParserType.BOOKSTACK.value: naive
}
...
async def run_bookstack_chapter_doc(task, progress_callback):
    ...
    """
    try:
        # Get config from KB parser_config
        kb_id = task["kb_id"]
        tenant_id = task["tenant_id"]

        # Get booknames from task config
        booknames_filter = task.get("booknames", [])
        if not booknames_filter:
            progress_callback(-1, "No booknames provided for BookStack chapter fetch")
            return []

        # Use minimal config - only fetch chapters and pages for specified books
        bookstack_config = {
            "include_books": False,
        ...
        progress_callback(0.1, f"Fetching chapters for books: {', '.join(booknames_filter)}")

        # Use standalone function
        return await fetch_bookstack_content(
            bookstack_config=bookstack_config,
            kb_id=kb_id,
            tenant_id=tenant_id,
            progress_callback=progress_callback,
            doc_id=None,  # Generate virtual doc_ids
            booknames_filter=booknames_filter
        )
